...
- **shot_slots**: projectId, chapterId, index, type (`stock|archival|still|chart|map|hero`), brief jsonb (the full creative brief, typed per §7 of the product spec), candidates jsonb[], chosenAssetId, status (`unresolved|resolved|placeholder`), startMs, durationMs. **`hero` (AI video) is feature-flagged off at launch** — the schema, brief type and UI badge exist, but no video-generation adapter is built until the flag is turned on post-monetisation; the shot-list prompt is instructed not to emit hero slots while the flag is off. *(Amended 2026-09-22, decision 268 (Plan B): `graphic` joins the type list, a composed scene rather than a photograph (`packages/schemas/src/graphics.ts`): up to six elements from a fixed vocabulary, text, figure, logo, shape, bars, on a 12 by 12 grid, coloured only by Brand Kit token NAMES and sized by a type role, never a hex value or a chosen pixel size. Every figure and every bars item cites a claim; the digits it shows are checked in code against that claim's own text. A logo names its entity and joins the uploaded logo library (decision 268); a mark not yet held leaves the slot a placeholder with an upload button on the card, never a refusal. Migration 0027 adds the one enum value.)*
...
```
`ChartPayload` embeds the resolved data series and the source claim IDs. Compositions consume this JSON and the brand snapshot only — a timeline uploaded to S3 plus the site bundle is a complete, reproducible render definition. *(Amended 2026-09-22, decision 268 (Plan B): the payload union gains `GraphicPayload`, embedded whole like a chart's series: the resolved scene, the logo image each `logo` element draws keyed by ELEMENT id, and the ids of every claim it cites, so a render never depends on the logo library or the claims table still holding what it read.)*
