...
export type IngestOutcome =
  { ok: true; r2Key: string; assetId: string } | { ok: false; reason: string }
 ...
 * candidate, no bytes in R2 yet, and a fetchable source. Pure — the runner
 * uses it to decide which slots get an ingest step at all.
 */
export function needsStockIngest(slot: StockIngestSlot): boolean {
  ...
  if (!candidate || candidate.r2Key !== undefined) return false
  return /^https?:\/\//.test(candidate.sourceUrl)
}
...
export async function downloadStock(
  ...
  return { ...second, url: fresh.sourceUrl }
}

/**
 * Ingest one slot's chosen candidate: bytes into R2, an asset row, and the
 * key written back into the slot's candidates jsonb — so a re-run, the
 * board, and every later timeline version all see the stored copy.
 */
export async function ingestSlotStock(slot: StockIngestSlot): Promise<IngestOutcome> {
  ...
  const ext = stockExtension(downloaded.contentType, downloaded.url)
  ...
  const updated = slot.candidates.map((entry) =>
    entry['chosen'] === true ? { ...entry, r2Key: key, assetId: asset.id } : entry,
  )
  ...
  return { ok: true, r2Key: key, assetId: asset.id }
}
