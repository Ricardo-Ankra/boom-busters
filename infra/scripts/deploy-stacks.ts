...
if (!brokerToken) {
  fail('BROKER_TOKEN is not set and ~/.boom-busters-broker-token does not exist (openssl rand -hex 32)')
}

const accountId = dotenv['R2_ACCOUNT_ID']
if (!accountId) fail('R2_ACCOUNT_ID missing from .env.local')

const required = (name: string): string => process.env[name] ?? fail(`${name} must be exported (printed by deploy-remotion.ts)`)

const config: Record<string, string> = {
  BROKER_TOKEN: brokerToken,
  CALLBACK_URL: process.env['CALLBACK_URL'] ?? 'https://boom-busters-web-rho.vercel.app/api/hooks/broker',
  ...
  R2_SECRET_ACCESS_KEY: dotenv['R2_SECRET_ACCESS_KEY'] ?? fail('R2_SECRET_ACCESS_KEY missing from .env.local'),
...
console.log(`deploying with CALLBACK_URL=${config['CALLBACK_URL']}, ALERT_EMAIL=${config['ALERT_EMAIL']}`)

const result = spawnSync('pnpm', ['exec', 'cdk', 'deploy', '--all', '--require-approval', 'never'], {
  cwd: path.join(here, '..'),
