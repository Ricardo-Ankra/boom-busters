...
export function e2eDatabaseUrl(): string {
  ...
  throw new Error(
    [
      ...
      '',
      'Start the local test container (see packages/db/src/test-database.ts), add its',
      'connection string to .env.local as TEST_DATABASE_URL and apply the schema with',
      '`pnpm db:migrate:test`. Never point this at Neon: a cloud test database eats',
      "the project's data-transfer quota, and production shares that quota.",
      '',
